...
exports.runWaterSimulation = void 0;
 ...
 */
function getKcForDay(day, crop) {
    if (day <= 30)
        return crop.kc_ini;
    if (day <= 60)
        return crop.kc_ini + ((crop.kc_mid - crop.kc_ini) * (day - 30) / 30);
    if (day <= 120)
        return crop.kc_mid;
    return crop.kc_mid + ((crop.kc_end - crop.kc_mid) * (day - 120) / 60);
}
// --- Main Simulation Function ---
const runWaterSimulation = (lat_1, lon_1, soilType_1, cropCategory_1, ...args_1) => __awaiter(void 0, [lat_1, lon_1, soilType_1, cropCategory_1, ...args_1], void 0, function* (lat, lon, soilType, cropCategory, interventions = [] // ['drip', 'mulch']
) {
    ...
    if (interventions.includes('drip'))
        efficiencyFactor *= 0.6;
    if (interventions.includes('mulch'))
    ...
    let dailyRain = new Array(simulationDays).fill(0);
    let dailyET0 = new Array(simulationDays).fill(4); // Default 4mm/day if fetch fails
    try {
        // A. Forecast (Days 1-15)
        const forecastRes = yield axios_1.default.get('https://api.open-meteo.com/v1/forecast', {
            params: {
        ...
        if (forecastRes.data.daily) {
            (forecastRes.data.daily.precipitation_sum || []).forEach((v, i) => { dailyRain[i] = v || 0; });
            (forecastRes.data.daily.et0_fao_evapotranspiration || []).forEach((v, i) => { dailyET0[i] = v || 4; });
        }
        ...
        const histRes = yield axios_1.default.get('https://archive-api.open-meteo.com/v1/archive', {
            params: {
                latitude: lat, longitude: lon,
        ...
        if (histRes.data.daily) {
            (histRes.data.daily.precipitation_sum || []).forEach((v, i) => {
                if (16 + i < simulationDays)
                    dailyRain[16 + i] = v || 0;
            });
            (histRes.data.daily.et0_fao_evapotranspiration || []).forEach((v, i) => {
                if (16 + i < simulationDays)
                    dailyET0[16 + i] = v || 4;
            });
        }
    ...
    for (let day = 0; day < simulationDays; day++) {
        ...
        if (newBalance <= wiltingPoint_mm) {
            ...
            if (firstStressDay === null)
                firstStressDay = day + 1;
            // Floor at zero
            if (newBalance < 0)
                newBalance = 0;
        }
    ...
    // Cache for 3 hours
    cache_1.apiCache.set(cacheKey, result, 60 * 60 * 3);
    return result;
});
exports.runWaterSimulation = runWaterSimulation;
