...
export interface YieldAdjustment {
    ...
    waterRatio: number;               // available / required
    stressSeverity: 'NONE' | 'MILD' | 'MODERATE' | 'SEVERE' | 'EXTREME';
    recommendations: string[];
}

export type CropCategory = 'CEREAL' | 'CASH_CROP' | 'PULSE' | 'VEGETABLE' | 'HORTICULTURE';

export class YieldAdjustmentService {

    // Stress Response Curves (Simplified FAO Ky Logic)
    // Key: Water % Availability, Value: Yield Factor (0-1)
    private static STRESS_CURVES: Record<CropCategory, Record<number, number>> = {
        'CEREAL': { // Wheat, Jowar - Moderate tolerance
            100: 1.00, 90: 0.95, 80: 0.88, 70: 0.78, 60: 0.65, 50: 0.48, 40: 0.28, 30: 0.10
        },
        'CASH_CROP': { // Sugarcane - Very Sensitive
            100: 1.00, 90: 0.92, 80: 0.82, 70: 0.68, 60: 0.50, 50: 0.28, 40: 0.12, 30: 0.05
        },
        'PULSE': { // Chana, Tur - Resistant
            100: 1.00, 90: 0.96, 80: 0.91, 70: 0.84, 60: 0.75, 50: 0.62, 40: 0.45, 30: 0.25
        },
        'VEGETABLE': { // Onion, Tomato - Sensitive
            100: 1.00, 90: 0.94, 80: 0.85, 70: 0.73, 60: 0.58, 50: 0.38, 40: 0.18, 30: 0.05
        },
        'HORTICULTURE': { // Pomegranate - Perennial Sensitive
            100: 1.00, 90: 0.93, 80: 0.84, 70: 0.71, 60: 0.54, 50: 0.32, 40: 0.15, 30: 0.05
        }
    };

    static adjustYieldForWaterStress(
        baseYield: number,
        ...
        if (waterRatio > 1.2) waterRatio = 1.2; // Cap excess benefit
        if (waterRatio < 0) waterRatio = 0;
        ...
        if (lowerBound < 30) {
            ...
            yieldFactor = curve[lowerBound as 100] || 0.1;
        } else {
        ...
        // Determine Severity
        let severity: YieldAdjustment['stressSeverity'] = 'NONE';
        const recs: string[] = [];
    ...
    static getCropCategory(cropId: string): CropCategory {
        const id = cropId.toLowerCase();
        if (id.includes('sugarcane') || id.includes('cotton')) return 'CASH_CROP';
        if (id.includes('gram') || id.includes('tur') || id.includes('moong') || id.includes('soybean')) return 'PULSE';
        if (id.includes('onion') || id.includes('tomato') || id.includes('potato') || id.includes('chilli')) return 'VEGETABLE';
        if (id.includes('pomegranate') || id.includes('grapes') || id.includes('mango')) return 'HORTICULTURE';
        return 'CEREAL'; // Default (Wheat, Rice, Maize, Bajra)
    }

    static predictYieldRange(baseYield: number, cropType: CropCategory, historicalRainfall: number[]): any {
