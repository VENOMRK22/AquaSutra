...
import graceDataRaw from '../data/grace_india.json';

// Type definition for the imported JSON
const graceData = graceDataRaw as Record<string, Array<{ lat: number; lon: number; anomaly_cm: number }>>;

export interface GRACEAnomaly {
    anomaly_cm: number; // Liquid Water Equivalent Thickness deviation
    date: string;       // YYYY-MM
    trend_cm_per_year?: number;
    uncertainty?: number;
}

export class NASAGRACEService {

    /**
     * Get Groundwater Anomaly for a specific location and date
     * Checks local JSON database and interpolates between grid points.
     */
    static getGroundwaterAnomaly(lat: number, lon: number, date?: Date): GRACEAnomaly | null {
     ...
     */
    static getAnomalyTrend(lat: number, lon: number, months: number = 6): number {
     ...
     */
    private static interpolateAnomaly(lat: number, lon: number, grid: Array<{ lat: number; lon: number; anomaly_cm: number }>): number {
        ...
        const neighbors = grid
            .map(p => ({ ...p, dist: Math.sqrt(Math.pow(p.lat - lat, 2) + Math.pow(p.lon - lon, 2)) }))
